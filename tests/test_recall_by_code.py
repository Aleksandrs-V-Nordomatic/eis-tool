...
def policy(**kw):
    ...
    base.update(kw)
    return batch.load_policy(__import__("json").dumps(base))
...
class ACodeCanRecall(unittest.TestCase):
    ...
    def test_a_short_title_with_an_exact_code_is_no_longer_invisible(self):
        ...
        self.assertFalse(batch.outside_scope(notice("Stebejimo sistema", "32323500"), rules))
    ...
    def test_without_the_code_list_that_notice_is_still_dropped(self):
        ...
        self.assertTrue(batch.outside_scope(notice("Stebejimo sistema", "32323500"), policy()))
    ...
    def test_a_notice_that_matches_neither_is_still_dropped(self):
        rules = policy(recall_cpv_prefixes=["32323"])
        self.assertTrue(batch.outside_scope(notice("Sviezios darzoves", "15331000"), rules))
...
class RecallNeverBeatsAnExclusion(unittest.TestCase):
    ...
    def test_an_all_excluded_code_set_still_wins(self):
        ...
        self.assertTrue(batch.outside_scope(notice("Tonometras", "33100000"), rules))
...
class TheShapeStaysBackwardCompatible(unittest.TestCase):
    ...
    def test_a_three_field_policy_tuple_is_still_accepted(self):
        ...
        self.assertFalse(batch.outside_scope(notice("Valdymo sistemos darbai", "45331000"),
                                             legacy))
        self.assertTrue(batch.outside_scope(notice("Sviezios darzoves", "15331000"), legacy))

    def test_the_committed_example_still_parses(self):
        with open(os.path.join(os.path.dirname(os.path.dirname(
                os.path.abspath(__file__))), "lt_policy.example.json"), encoding="utf-8") as fh:
            rules = batch.load_policy(fh.read())
